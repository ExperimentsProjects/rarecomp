...
export async function GET(){ await dbReady();const admin=await getAdmin();const existing=await db.select({id:admins.id}).from(admins).limit(1);return NextResponse.json({admin,needsSetup:existing.length===0});}
export async function POST(req:Request){ await dbReady();
 ...
 const key=email.toLowerCase().trim();const attempt=attempts.get(key);if(attempt&&attempt.until>Date.now()&&attempt.count>=8)return NextResponse.json({error:'Too many attempts. Please try again in 15 minutes.'},{status:429});
  ...
  }catch(e){console.error('Admin session error:',e);const msg=e instanceof Error?e.message:'';if(/DATABASE_URL|reach PostgreSQL|password/i.test(msg))return NextResponse.json({error:msg},{status:503});return NextResponse.json({error:'Unable to sign in. Please try again.'},{status:500});}}
