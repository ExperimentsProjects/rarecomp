...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (typeof currentPassword !== 'string' || typeof password !== 'string' || password.length < 8 || password.length > 128)
      return NextResponse.json({ error: 'Use a new password of 8–128 characters.' }, { status: 400 });
    ...
    await db.update(admins).set({ passwordHash: hashPassword(password) }).where(eq(admins.id, admin.id));
