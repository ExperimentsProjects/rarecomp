...
export async function GET(){ await dbReady();if(!await getAdmin())return NextResponse.json({error:'Unauthorized'},{status:401});return NextResponse.json(await db.select().from(subscribers).orderBy(desc(subscribers.createdAt)));}
